...
def tickerBtc():
    ...
    return RestJson.ticker(url, True)


def ticker(coinName):
    rootUrl = "https://api.bithumb.com/public/ticker/{}"

    url = rootUrl.format(coinName)
    return RestJson.ticker(url, True)
...
def TradeTicker(length, timespan):
    ...
    for i in range(length):
        time.sleep(timespan)
        jsonData = tickerBtc()
        priceArray.append(TickerModel(jsonData, "BTC", "KRW"))
...
'''
def TickerModel(jsonData, site="bithumb", setCoin = "btc", buyCoin = "usd"):
